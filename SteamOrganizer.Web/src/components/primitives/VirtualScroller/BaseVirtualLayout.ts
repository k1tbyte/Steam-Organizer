...
/*        let element: HTMLElement | null = this.scroller.querySelector(`[virtual-wrapper]`);
        ...
        this.offsetBefore = wrapperTop - scrollerTop + this.scroller.scrollTop;

        const sample = this.list.children[0];
        ...
        this.rowHeight = sample.clientHeight +
            this.rowGap +
            (parseFloat(cellStyle.borderTopWidth) || 0) +
            (parseFloat(cellStyle.borderBottomWidth) || 0) +
            (parseFloat(cellStyle.marginTop) || 0) +
            (parseFloat(cellStyle.marginBottom) || 0);
