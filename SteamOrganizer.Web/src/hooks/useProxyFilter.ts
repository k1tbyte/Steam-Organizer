import {ObservableCollection, ObservableObject} from "@/lib/observer/observableObject.ts";
...
export const useProxyFilter = <T>(
    ...
    const applyFilter = (filter: (data: T[]) => T[]) => {
        filterRef.current = filter;
        state.proxy.proxyCallback(state.collection.value);
    };

    return [applyFilter, state.collection, state.proxy] as const;
};